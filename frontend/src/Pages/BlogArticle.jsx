...
function BlogArticle() {
  ...
  return (
        ...
        <div className='mt-3 flex gap-3 flex-wrap'>
          {
            categories.map((item) => (
              <span
                ...
                className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${category === item ? 'bg-purple-200 text-purple-700' : 'text-gray-500 border-gray-300' }`}
                key={item}
              >
                {item}
              </span>
    ...
    <button className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#C341F6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
      {
       loading ? (<Loader size={24}/> ) : <Hash className='w-5' /> 
      }Generate title
    </button>
      </form>
